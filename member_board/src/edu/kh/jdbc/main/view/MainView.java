		...
		do {
			try {
				...
				if(loginMember ==null) {
					...
					case 0 :  // 로그아웃 == loginMember 가 참조하는 객체 없음( == null)
...
//							input = 0; // -> do-while 조건식을 false로 만듦(종료)
							System.exit(0);//JVM 종료ㅡ 매개변수는 0, 아니면 오류를 의미
							break;
					default : System.out.println("메뉴에 작성된 번호만 입력해주세요.");
					}
	...
	private void signUp() {
		...
		try {
			...
			while(true) {
				...
				String gender = sc.next().toUpperCase(); // 입력받자마자 대문자로 변경
				
				if(gender.equals("M") || gender.equals("F")) {
					
					break;
				} else {
					System.out.println("F또는 M을 입력해주세요");
				}
				System.out.println();
				}
	...
	try {
	    ...
	    *      (게시글 번호, 제목[댓글수], 작성자명, 작성일, 조회수, 댓글 수)
	    * 
